...
TestingSessionLocal = sessionmaker(
    autocommit=False, autoflush=False, bind=engine_test_db
)
...
async def test_like_tweet(setup_and_teardown):
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            "/tweets",
            json={"tweet_data": "This is a tweet to be liked", "tweet_media_ids": []},
            headers={"api-key": test_user.api_key},
        )
...
async def test_unlike_tweet(setup_and_teardown):
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            "/tweets",
            json={"tweet_data": "This is a tweet to be unliked", "tweet_media_ids": []},
            headers={"api-key": test_user.api_key},
        )
        tweet_id = response.json()["tweet_id"]
